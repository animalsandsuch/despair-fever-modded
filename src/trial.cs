...
function dropFoods()
{
	...
	while(%foodCount >= 0)
	{
		...
		%brick.setItem(getRandom() > 0.1 ? pickField("BananaItem" TAB "ViceSmokeItem") : "BurgerItem"); //I'm too lazy to check if the brick already has food spawned sooo....
		%foodCount--;
	}
	$DefaultMiniGame.chatMessageAll('', '\c3~~ Food has been restocked!\c6 Go eat your cheeseburgers. Oh, and cigarettes, too.');
}
...
function courtPlayers()
{
	...
	if(isObject($mangled))
		ServerPlaySong(pickField("DespairMusicWonderfulIntro"));
	else
