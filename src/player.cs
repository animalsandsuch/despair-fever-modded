...
datablock PlayerData(ViceItemsArmor : PlayerStandardArmor) {
	shapeFile = "base/data/shapes/player/hands.dts";
...
package ViceItemsPackage {
	...
	function GameConnection::applyBodyParts(%cl) {
		...
		if (isObject(%cl.player.ViceItemBot)) {
			applyViceItemProperties(%cl.player.ViceItemBot, 1);
		}
		return %ret;
	}
	...
	function serverCmdUseTool(%cl, %slot) {
		if (%cl.player.tool[%slot].botDatablock $= "ViceItemArmor") {
			if (getSimTime() - %cl.lastUseTime < 100) {
				cancel(%cl.equipItemSchedule);
				%cl.equipItemSchedule = schedule(150, %cl, serverCmdUseTool, %cl, %slot);
				return;
			}
			%cl.lastUseTime = getSimTime();
		}

		parent::serverCmdUseTool(%cl, %slot);
	}
};
activatePackage(ViceItemsPackage);


////////////////////


function equipViceHands(%pl, %botDatablock) {
	if (!isObject(%pl.ViceItemBot)) {
		%pl.ViceItemBot = new AIPlayer(ViceItemBots) {
			datablock = %botDatablock;
			player = %pl;
		};
		%pl.ViceItemBot.kill();
	}
	if (!isObject(%pl.emptyBot)) {
		%pl.emptyBot = new AIPlayer(ViceItemBots) {
			datablock = %botDatablock;
		};
		%pl.emptyBot.kill();
		%pl.emptyBot.hideNode("ALL");
	}
	%pl.ViceItemBot.setScale(%pl.getScale());

	%pl.mountObject(%pl.emptyBot, 0);
	%pl.emptyBot.mountObject(%pl.ViceItemBot, 0);
	%pl.playThread(1, armReadyBoth);

	applyViceItemProperties(%pl.ViceItemBot);
}

function applyViceItemProperties(%obj, %bypassApply) {
	%pl = %obj.player;
	%obj.hideNode("lHand");
	%obj.hideNode("rHand");
	%obj.hideNode("lHook");
	...
	if (!isObject(%pl.client)) {
		...
		%obj.setNodeColor("rHand", "1 0.878 0.611 1");
		%obj.setNodeColor("lHand", "1 0.878 0.611 1");
	...
	if (isFunction("GameConnection", "getTeam") && isObject(%pl.client.getTeam())) {
		%team = %pl.client.getTeam();
