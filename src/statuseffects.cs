	...
	switch$ (%effect)
	{
		...
		case "hankering":
			if(%slot != $SE_passiveSlot)
				return %slot;
			cancel(%player.statusSchedule[%slot]);
			%player.statusSchedule[%slot] = %player.schedule(60000, updateStatusEffect, %slot);
		...
		case "bleeding":
			...
			if(!%nomsg && isObject(%player.client) && %player.statusEffect[%slot] !$= "bleeding")
				if(%player.character.trait["Masochist"])
				%player.addMood(3, "You are " @ getStatusEffectColor(%effect) @ "bleeding\c5... So good...", 1);
			else
...
function Player::updateStatusEffect(%player, %slot)
{
	...
	switch$ (%effect)
	{
		...
		case "hankering":
			cancel(%player.statusSchedule[%slot]);
			%player.addMood(-2, "You really need a smoke...", 1);
			%player.statusSchedule[%slot] = %player.schedule(60000, updateStatusEffect, %slot);
